...
- **`.github/workflows/release.yml`** (PR [#15](https://github.com/XMV-Solutions-GmbH/cert-manager-webhook-hcloud-zones/pull/15), part of [#7](https://github.com/XMV-Solutions-GmbH/cert-manager-webhook-hcloud-zones/issues/7)) — release pipeline triggered on every `v*` tag push. Builds a multi-arch container image (`linux/amd64` + `linux/arm64`) and pushes it to GHCR; signs the image with cosign keyless signatures via Sigstore; generates and attaches an SBOM (anchore/sbom-action) to the GHCR image; packages the Helm chart and pushes it to `ghcr.io/<owner>/charts` as an OCI artefact; and creates a GitHub Release with auto-generated release notes. Image signing and SBOM attachment each have their own `id-token: write` grant for OIDC; the top-level `permissions` block is narrowed to `contents: read`.
